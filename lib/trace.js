  ...
  return resolve(depName, {
    ...
    packageFilter: (pkg, pkgfile) => {
      ...
      if (!pkgPaths.length) {
        ...
        CONDITIONS.forEach((cond) => {
          ...
          Object.entries(PASSTHROUGH_EXPORTS).forEach(([key, val]) => {
            if ((pkg.exports || {})[key] === val) {
    ...
    .catch((err) => {
      if (
        ...
        && pkgPaths.length === 0
        && exportSrcs.size === 0
        // TODO: Make a general handler function
        // TODO: Unit tests for normal package
        // TODO: Unit tests for scoped package
        // TODO: Make sure not hitting source files
        && (
          depName.startsWith("nomain/")
          || depName.startsWith("jose-node-cjs-runtime/")
        )
        && !addRootPackagePaths
      ) {
