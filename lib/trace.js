...
const isAllowedMiss = ({ depName, srcPath, allowMissing }) => {
  ...
  if (
    matchesAllowed({ depName, allowed: allowMissing[path.resolve(srcPath)] })
  ) {
    return true;
  }
...
const _resolveDep = async ({
  ...
  // Support advanced Node.js ESM import types.
  if (IGNORE_IMPORT_PREFIXES.find((prefix) => depName.indexOf(prefix) !== -1)) {
    return null;
  }
  ...
  return (
    resolve(
      queryStringPosition === -1
        ? depName
        : depName.substring(0, queryStringPosition),
      {
        basedir,
      ...
      .then(async (depPath) => {
        ...
        const exportPaths = await Promise.all(
          Array.from(exportSrcs.values()).map(async ({ relPath, fullPath }) => {
...
const traceFile = async ({
  ...
  const depPaths = uniq(
    depObjs.reduce(
      (memo, { depPath, exportPaths }) =>
        memo.concat(depPath || [], exportPaths),
      []
    )
  );
  const allPkgPaths = uniq(
    depObjs.reduce((memo, { pkgPaths }) => memo.concat(pkgPaths), [])
  );
