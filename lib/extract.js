...
// Customize the visitor to handle things `node-walk` doesn't presently
// handle.
...
// Traverse into the value of the node.
const descendValue = (node, st, c) => c(node.value, st);
const walkVisitor = Object.assign({}, acornWalk.base, {
  FieldDefinition: descendValue
});

// Manually re-create the acorn walk function to allow better customizability.
// TODO: Consider removing if walkVisitor gets everything done...
const customWalk = (
  node,
  visitors,
  baseVisitor = acornWalk.base,
  state,
  override
) => { // eslint-disable-line max-params
  // Create traversal function.
  const descend = (dNode, dState, dOverride) => {
    const type = dOverride || dNode.type;
    const found = visitors[type];

    // TODO: DEBUG REMOVE
    if (!baseVisitor[type]) {
      // eslint-disable-next-line no-console
      console.error("TODO HERE MISSING TYPE", { type });
    }

    baseVisitor[type](dNode, dState, descend);
  ...
  // Initial call.
  descend(node, state, override);
};

let walk = acornWalk.simple;
walk = customWalk; // TODO REMOVE.
